...
import { Calendar, DollarSign, Users, Tag, Info, ChevronDown, ChevronUp, ShoppingBag, Utensils, Car, Film, Home, Lightbulb, Plane, Package, HeartPulse } from 'lucide-react';
...
const ExpenseCard = ({ expense, members, currentUserId = '1', onPaymentClick }: ExpenseCardProps) => {
  ...
  return (
    <div className="card-base overflow-hidden transition-all hover:shadow-elevate">
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          {userStatus && (
            <div className={`text-sm ${isUserPayer ? 'text-green-600' : userSplit?.paid ? 'text-green-600' : 'text-amber-600'}`}>
              {userStatus}
            </div>
          )}
        </div>
        ...
        <div className="mt-4 animate-slide-down">
          ...
          <div className="space-y-2">
            ...
            <div className="flex -space-x-2 overflow-hidden">
              {members
                .filter(member => expense.splits.some(split => split.memberId === member.id))
                .map(member => (
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="space-y-1.5">
                {expense.splits.map(split => {
                  ...
                  return (
                    <div key={split.memberId} className="flex justify-between items-center text-sm py-1">
                      <div className="flex items-center">
                      ...
                      <div className="flex items-center">
                        <span className={split.paid ? 'text-green-600' : 'text-amber-600'}>
                          {formatCurrency(split.amount, expense.currency)}
                        </span>
                        {!split.paid && member.id === currentUserId && onPaymentClick && (
                          <Button 
                            size="sm" 
                            variant="outline" 
                        ...
                        {(split.paid || !onPaymentClick || member.id !== currentUserId) && (
                          <Badge variant={split.paid ? "secondary" : "outline"} className="ml-2 text-xs">
